...
## 本会话新增(2026-06-18 第四段:★漏斗 v3 质量裁定 → 架构对·两硬伤)
把上段的 v3「质量未裁」补上。从 test2 全量 `labels_auto.json` 按 干预×危害密度×负样本/边界 切**分层
样本 28 份**(`eval/funnel_sample.py`→`_funnel_sample.json`,跨~20 部门破航空偏),跑 `funnel.run_funnel`
(flash 单跑,`eval/run_funnel_eval.py`→`out_funnel/`),**Opus 逐份独立中立裁**(只信原文,auto-label 仅分层
非真值,rubric=`eval/funnel_adjudicate_rubric.md`,裁定=`eval/funnel_verdicts.json`)。完整报告
`eval/funnel_v3_adjudication_2026-06-18.md`。
- **结论:v3 架构骨架对(R0 一击挡住 15/15 纯规则负样本、残差 1 轮收敛不震荡、工程类政策精度高),
  但两硬伤拦着它现在替代 `analyze()`。**
- **指标**:① **R0 门 74.1%(20/27)**:**R0 漏报 5**(石化稳增长/城际铁路/浙江自贸/林下经济/消防取消认证)
  + R0 假阳 2(矿山普查/海上风电用海)。② **精度 46%(42/91 valid)**,高度分化:节能降碳改造 16/16、
  风电退役 3/3(真工程锚定扎实)vs 社区卫生 1/9、互助养老 4/16(**效益/服务政策硬造危害**)。
  ③ **假阳类型:硬造危害 28(压倒性)> 背景非干预 12 > 应激泛化 10**;跨段/锚定失败≈0
  (→ valid() 的 A 锚定、C 机制守住了,失守在 **B 干预归因 + 反转效益硬造**)。④ 召回最大杀手=R0 漏报
  (5 份整份 0 召回·高 severity),非 R1/R3 找不全。
- **R0 漏报规律(关键)**:① 制度措辞包裹实质建设/产能("稳增长"含扩产能、"健康可持续发展"实指财务但
  正文是新建线路、"枢纽建设"含油气化工管网)→ R0 被表层措辞带偏没穿正文;② 放松防护标准型(取消消防
  强制认证)误判非干预。**危害转向里漏一份高危政策比多几条假阳更伤(整份体检失效)。**
- **★阶段归因(R0–R6,别只盯 R0)**:精度 46% 不是模糊整体缺陷,而是**一个具体的 R2 失效被 R3 放大**。
  数据:R2 候选 114→留 91 只砍 20%、留存仅 42 成立 → **R2 漏 49**(valid 的 A 锚定/C 机制守住了,
  失守在 **B 干预归因 + 无反转效益判**;效益政策上 R2 砍≈0)。R1 没问题(过度生成是设计预期);
  R3 不是独立问题(效益政策多轮空转滚雪球,**修好 R2 自动安全**,单向残差结构不动);**R5 的 strength
  救不了场**——反直觉:模型对自编硬造标"中"、对真危害标"推测",**效益期"删推测"确定性剪枝在危害上失效**。
- **下一步(按 ROI,均改提示+重测同 28 份,守纪律别凭推理当定论)**:① **R2 valid() 加 B 干预归因严判
  + 反转效益/执行失败门(最高杠杆单点——一并解精度瓶颈+R3 放大)**:危害须由政策主动引入的干预直接产生,
  不得由'执行不到位/监管不力/服务差'反推、不得反转改善事项;② 补 R0 漏报(独立战线·提示加"穿透制度措辞、
  扫正文重大项目/扩产能/新建改建/新设园区/放松防护→判干预"+标题安全网);③ R0 补"纯信息调查/审批指标
  口径≠干预"(治普查/用海假阳)。**注:R2(路径级门)与 R0(政策级门)互不替代,两条线都要。**

## 本会话新增(2026-06-18 第五段:★R0 重构 = 整篇布尔→逐条摘干预条款 + 喂 R1)
承上段阶段归因(R0 漏报+R2 是瓶颈),先动 R0。**根因**:R0 旧版是"读全文一次性下布尔",被宏观/制度主调
带偏(石化"稳增长"含新设园区/建成投产却判无干预),且"园区认定管理…扩容或新设立"的认定外壳还撞上旧提示
否定清单的"资质认定"。**改法(`funnel.py` SYS_R0/SYS_GEN/run_funnel)**:R0 从"下判断"改为"逐条摘录
【引入/实质改变健康决定因素条件】的干预条款(逐字 quote+change),空表→gate out";**抽出的条款直接作 R1 输入
(R1 不再全文重读=省一次全文输入+把候选锚定在真实干预上)**;R3 仍读全文兜底召回。
- **对照测 8 份(`eval/test_r0_refactor.py`→`out_funnel_v4/`)**:① 石化 R0 漏报**捞回**(摘 19 条款→R1 逐条 19 危害,
  全真;注:批量首跑出 0 是 **flash 抽空方差**,重跑 19,非 bug);② **两个 R0 假阳(矿山普查/海上风电)一并 gate 掉**
  (摘不出实质条款→空表),海上风电连带 6 条假危害消失;③ 节能降碳(16)/国债期货/春运 **不打乱**;
  ④ **额外红利**:社区卫生硬造 **9→6 且全真**——R1 锚定真干预条款从源头压住 R2 最头疼的硬造。
- **仍剩(下一步)**:① **R2 还在橡皮图章**(石化 19 候选 R2 全 keep,边际弱条款"消防队→灭火剂"也不砍)——
  阶段归因钉的精度瓶颈未动,下一步=R2 valid() 加 B 干预归因严判+反转效益门+砍边际;② **flash 方差**(石化 0↔19)
  对体检工具是硬伤,评测/上线前须多跑取稳或换 pro 终判;③ 消防取消强制认证仍 gate(认证管理 vs 放松防护的定义
  边界,待用户拍板);④ 全 28 份用新 R0 重跑拿聚合数(顺便多跑压方差)未做。
- 入库:`funnel.py`(R0/R1 改)+`eval/test_r0_refactor.py`;运行时产物 `out_funnel_v4/` gitignore。

## 本会话新增(2026-06-18-19 第六段:★漏斗主干补全 R2–R7 + max_tokens 实锤)
承第五段(R0 已修、R2 仍橡皮图章),把整条 funnel 主干按"每阶段一个干净职责"补全并逐段验证。
**线上仍旧效益版;funnel 仍是离线原型,未接 `app_nicegui`。**当前管线(`funnel.py`):
```
R0 抽干预条款 → R1 据条款生成(召回) → R2 杀路径(精度) → R3 召回桶(不走R2)
→ R4 杀桶(清畸形+R2原则) → R5 语义合并(LLM) → R6 富化 → R7 聚合(compute_items)
```
- **R2 = 杀路径,轴=「特定 vs 泛化」不是「固有 vs 执行失败」**(`SYS_VALID` 重写):一条危害成立 ⟺ 它是
  该类干预【特定、可预见】的危害——正常工况固有影响(拆除扬尘/装置排放)**与**特征性事故(化工泄漏/溃坝)**都留**;
  泛化套链(失业/价格/生态→健康)**与**泛泛办砸(服务差/系统误判→出事)**都杀**。「可被措施避免/施工期临时」绝非
  剔除理由(=措施缺口)。**关键教训**:曾用「固有 vs 执行失败」二分→误杀化工泄漏(它属"非正常工况"却是 HIA 核心),
  且 flash 把"拆除必有粉尘"误读成"执行失败"全杀光(节能降碳 16→0)→ 加正向豁免"正常实施固有副产物必留"才修好。
- **R2/R3/R4 分工(用户定的技术路线)**:R2 从严杀(误杀不怕);**R3 召回桶产物不喂回 R2**(同一法官会同样杀=死锁),
  绕开收入 `recovered` 桶;**R4 复核桶 = ① 清畸形(`_looks_harm` 代码剔"政策原句回声/无健康终点")② 过 R2 同一原则**。
  实测关键结论:**R2 原则一旦校准对,绕开它的必要性基本消失**——桶里真危害(R1 漏生成的、非 R2 误杀的)过同一原则即通过,
  R4 无需对立视角。召回桶 = (a) R1 漏的真危害(节能降碳:干熄焦/氢气/石棉/二噁英,该捞)+ (b) 政策原句回声畸形(先剔)。
- **R5 = 一次 LLM 语义合并(`SYS_MERGE`,新)**:exact-sig `_sig` 漏的"换措辞同义危害"在此收口(消防×4/二噁英×3 折叠成 1)。
  出 groups+合并链,代表取最长原链。**石化 32→13、自贸 25→10**,合并准(8 条事故链→1、4 条排放链→1)。
- **★max_tokens 4000→20000(实锤,`_call`)**:探明 **deepseek-v4-flash 是推理模型,单调用烧 ~7k+ 思考 token**;
  4000 会把推理掐断→`content` 空→该阶段静默失败丢条。**这是我们一度当成"模型抽风/不可救方差"的一大块真因**——
  但**仅限大输出阶段**(R5 合并在石化 25 条上 4000 必失败、8000/20000 才成);**R0 是否 gate 的摇摆是真模型方差**
  (R0 输出小、不截断),这个修复管不到。API 实测接受到 131072 不报错;取 20000(实测最坏 7176 的~2.8×,cap 只按实际生成计费)+timeout 180。
- **诊断/纪律**:flash 是推理模型这点之前没意识到,导致长期错把截断当随机。**改 funnel 任何阶段后,先确认输出没被 max_tokens 截断**
  (看 finish_reason=length / content 空)再谈质量。单跑计数仍有真方差(R0 gate 摇摆),裁质量仍需稳样多跑或换 pro。
- **未做/下一步**:① **R0 gate 真方差**(真干预政策偶被判 0 条款 gate 掉=整份漏报,体检硬伤)——也许加轻量兜底(标题/正文
  强干预词安全网);② **R6 的 mitigation 是纯黑箱 LLM 判**(读 25k 原文判已含/不足/未提及,**无出处锚定**),与"可审计"卖点冲突,
  拟仿 R0/R1 改"判已含/不足须逐字摘录原文措施、摘不出即未提及"(低风险、一致);③ 大样本质量裁定(架构稳后)+ 接入运行时。
- 入库:`funnel.py`(R2/R3/R4/R5 + max_tokens)、`eval/run_funnel_v4.py`(线程池并发跑测)、`eval/funnel_sample.py`/
  `run_funnel_eval.py`/`test_r0_refactor.py`/`multirun.py`/`funnel_adjudicate_rubric.md`/`funnel_verdicts.json`/
  `funnel_v3_adjudication_2026-06-18.md`;运行时产物 `out_funnel_v4/`、`_funnel_sample.json` gitignore。
...
8. **★漏斗主干(`funnel.py`,第三~六段产出)**:R0–R7 八段已补全并逐段验证(详见"第六段"记录)。**剩下**:
   - ~~质量大样本裁定~~✅(第四段)、~~R0 重构~~✅(第五段)、~~R2 杀准/R3 召回桶/R4 复核/R5 合并~~✅、~~max_tokens 修复~~✅(第六段)。
   - **R0 gate 真方差**:真干预政策偶被 R0 判 0 条款 gate 掉(整份漏报,体检硬伤)→ 拟加轻量兜底(标题/正文强干预词安全网)。
   - **R6 mitigation 黑箱**:现纯 LLM 读 25k 原文判已含/不足/未提及,**无出处锚定**(与"可审计"冲突)→ 拟改"判已含/不足须逐字摘录原文措施、摘不出即未提及"。
   - **接入运行时**:funnel 仍离线原型,未接 `app_nicegui`;上面两点 + 大样本质量复裁过关后,再把 `funnel.run_funnel` 接成 `analyze` 的备选/替代管线。
   - **措施质量**:`measures` 未经专门质量审,作初筛草案够、定稿不够 → 须标「供专家核定」。
