...
## 本会话新增(2026-06-18 第三段:模板接运行时 + ★引擎架构大探索 → 漏斗 v3)
本段先把模板接进运行时,再被一连串对照实验逼着重审整个引擎架构,最后落到一个**漏斗式原型**。
**结论:得到一套更干净的架构(`funnel.py` v3),但仍是离线原型、未接入 `app_nicegui` 运行时、质量未大样本裁定。**
线上仍旧效益版,本段所有改动均未上线。

- **模板接运行时(`template_join.py` 新)**:chain → `determinants.resolve_all` 落枢纽 → 按 **(hub, outcome_q) 精确 join** `eval/templates_harm.json`。
  实测决策:resolve 在「措施文本」仅 16% 命中、在「整条 chain」~83%+ → **ID-join 接在 post-gen(展开后)的 chain 上,不接 hint_builder**。
  砍掉「同 hub 兜底」(命中的 17.5% 全是跨题错配)→ 只留精确 (hub,q),守「精准>召回」。
  **模板退回「参考」位**:挂到 `p["template_ref"]`(同类政策参考做法 + 措施缺口分布),**不覆盖 `measures`**(measures 始终引擎现场生成=定制,模板=参考 checklist,来源透明)。已接进 `hia_screen.analyze`(line ~525,annotate 前)。
- **`hia_screen._SYS_EXPAND` 措施口吻**:measures 始终生成,口吻随 mitigation(未提及→新增/不足→强化/已含→可进一步);增量措施不改变是否判定(判定只看 mitigation 缺口)。已验证。
- **★引擎架构探索(一连串对照实验,全部 flash vs pro,以 Opus 当中立裁判)**——把过往教训钉死,免重走:
  · **pro-direct 不安全**:pro 直接生成在**最严重的政策(工业园区,原文点名重金属/危化品)上只给 1 条、漏掉核心危害**。根因:**强模型忠实执行了一套为勒住 flash 过度生成而调的"克制"提示**(克制/重点找被忽视/别求全)→ 把"显而易见的"主动跳过("已有可能被识别")。**模型越强,越被为弱模型调的克制提示带偏。**
  · **提示词「补丁 vs 规格」**:为补某模型缺陷的"克制告诫"=补丁,会随模型腐烂;从任务定义导出的=规格。判据:**「换更好的模型还需要这句吗?」**。能写成「输出 = {x : valid(x)}」就别写「请别漏某类 x」。
  · **清洁≠更好(实测)**:把 `_SYS_EXPAND` 改成纯净"集合定义版"→ **回归**:flash 去掉克制后爆量(限行9/夜间13)、负样本「油气管网公平开放」pro 0→6(抽象判据挡不住"编一条技术上穿枢纽的链")、还误删 `status` 字段致 compute_items 判是全 0。**教训:克制/负样本门/status 都是承重,可移植性的解法是"搬不删"(搬到审计/剪枝/代码)+每步实测,不是裸删。**
  · **多重 flash 漏斗(残差思路,用户提出)= 正解方向**:把冲突目标(召回 vs 精度)拆到不同清洁阶段,补丁因"无冲突需调和"自然消失。
- **★漏斗 v3(`funnel.py`,定稿原型架构,模型无关)**:
  ```
  R0 干预性筛选(HIA Screening·政策级一次)──无干预→空集
  R1 生成(召回) → R2 valid()三判据(精度·过一次)
  R3 召回残差 loop-until-dry(残差核心·只此一向) → R4 去重(代码) → R5 富化 → R6 聚合(代码)
  ```
  · **R0 = HIA 定义性筛选**(用户洞察):HIA 对象按定义是「含**干预**的 proposed policy/program/project」。**「有无干预」比「健康相关性」更本质**(健康相关性太松,任何摩擦→应激→心理健康都能套);干预含**服务/资源/保护标准的增减**,不只物理建设。`R0` 一击解决负样本(信用修复/油气管网 → 干预否 → 0)。整条管线对得上 HIA 规范步(Screening→Scoping→Appraisal)。
  · **valid() 三判据**:A 原文锚定(逐字摘录·防幻觉) / B 归因于本政策干预(非既有背景) / C 穿中间性决定因素。
  · **★残差只用召回单向**:曾试 v2「双向 Δ+/Δ- 迭代到不动点」→ **整状态精度复审迭代会过度剪枝(03 适老化→0)且震荡不收敛**(LLM 增删非收缩,不像 ResNet 残差有梯度保证)。故**精度过一次、只让召回做残差循环**。
  · **7 份验证结果(flash)**:01→5、02→7(重金属保住)、03→5(v2 是 0)、04→7、05→2;**两负样本 R0→0**;**全部 R3 第 1 轮即收敛**(不震荡)。结构性结论稳;但**只验了数量/门控,质量(精度/召回 vs 真值)未裁**。
  · **未做**:① 质量大样本裁定需**分层代表性样本(~25–30,按 干预×危害密度×负样本/边界 分层;现有 100 份发改委偏监管、5 份合成偏高危害,池子本身偏)**;② v3 仍是**离线原型,未接入 `app_nicegui` 运行时**;③ 单跑 flash,计数有方差。
...
7. **★危害转向收尾(进行中,本会话核心)**:
   - **产品侧**:`build_screen_docx`/界面要展示每条危害的 `mitigation`(措施缺口)+ `measures`(建议措施)+ `template_ref`(同类参考)——HIA 交付物,引擎已产出但前端/docx 还没渲;`template_ref` 须标「供专家核定/同类参考」。
   - ~~**危害+措施模板蒸馏**~~ ✅ 已完成;~~**接进运行时 ID-join**~~ ✅ 已完成(`template_join.py`,挂 `template_ref` 不覆盖 measures)。
   - **上线**:危害转向 + flash/pro 需 `git push` → 服务器 `git pull && systemctl restart hia`(线上仍旧效益版)。
8. **★漏斗 v3 收尾(本会话第三段产出,`funnel.py`)**:
   - **质量大样本裁定**:配**分层代表性样本(~25–30,干预×危害密度×负样本/边界)**跑 v3 → Opus 中立裁精度/召回/R0 门准确率;重点盯 R0 误杀真干预(漏报)+ 应激/边际假阳。现有池子偏(100 发改委偏监管、5 合成偏高危害)。
   - **接入运行时**:v3 现为离线原型,未接 `app_nicegui`;质量过关后把 `funnel.run_funnel` 接成 `analyze` 的备选/替代管线。
   - **措施质量**:模板/现场 measures 均**未经专门质量审**(pro 只审了路径真伪),作初筛草案够、定稿不够 → 须标「供专家核定」,或飞轮加一轮措施专审。
9. **末段证据 ID-join**:`hia_evidence.match` 仍是关键词 substring;可比照 `template_join` 换成按 `determinants` 枢纽 ID 精确 join,证据卡补 hub_id。
