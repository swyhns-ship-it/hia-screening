...
9. 路径**总数控制在最关键的约 15 条以内**,优先列把握高、影响大的;少而准 > 多而杂。
...
只补**真正缺失**的健康因果链,严守:
- **不要重复或近似重复已有路径**(已列过的机制不要换措辞再列一遍);只补新机制/新视角/新脆弱群体。
- **不要把政策设计点评(有效期/罚款用途/监管/信息公开等)当路径**——那不是健康因果链。
- strength 从严:依赖较多假设的补充路径标"推测",不要虚高。
