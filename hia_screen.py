...
务必先判健康关联:
0. **逐行动判健康关联**:若某行动与人群健康**无实质因果关联**(纯财务/税收/表彰/人事/纪念/
   行政程序等),**不要为它编造任何健康路径**——该行动产生 0 条;若所有行动都与健康无关,
   pathways 直接返回空。**宁缺勿造,假阳性比漏掉更糟。**
...
def expand_pathways(actions, doc_text, key, fallback=False):
    ...
    extra = ("\n注:本政策已判定为健康相关,但未能逐条枚举具体行动(行动清单仅为政策主题)。"
             "请**直接依据文档内容**,系统展开该政策的主要健康影响路径(通常应有若干条,"
             "把握高的优先,**不要返回空或 0 条**)。" if fallback else "")
    user = (f"【行动清单】\n{alist}\n\n【评估对象文档(供引用原文)】\n{doc_text[:25000]}\n\n"
