...
③ **始终**给出**结合本政策背景、具体可操作的建议措施**(measures 字段),口吻随 ② 而变:
   · 未提及 → 「建议**新增**:…」;· 不足 → 「现有不足,建议**强化为**:…」;
   · 已含 → 「政策已含 X,**可进一步**:…」(即便已有措施也可从专业角度提增量)。
   注:是否提增量措施**不改变危害是否成立的判定**(判定只看 mitigation 缺口);measures 是增值建议。
...
只输出 JSON:{{"pathways":[{{
 ...
 "mitigation":"已含|不足|未提及","measures":"结合本政策背景的建议措施(始终给出·口吻随mitigation·1–2条)",
...
def analyze(doc_text, key, project_name="", progress=None, hint_builder=None):
    ...
    for i, p in enumerate(paths, 1):       # 过滤后重新编号 P1、P2…
        p["id"] = f"P{i}"
    # ★末段 ID-join:每条危害路径 chain → 决定因素枢纽 → 验证版危害模板,回填代表性建议措施
    # + 措施缺口分布(确定性、零 API、可复现;详见 template_join.py)。引擎已给 measures 时不覆盖。
    template_join.enrich(paths)
    _p("④ 匹配证据来源…")
